...
Android app that reads and rewrites the IMEI on a rooted **DuoQin F21 Pro** — same crypto and on-device flow as [`alltechdev/f21-imei-switcheroo`](https://github.com/alltechdev/f21-imei-switcheroo), wrapped in a small native UI so you don't need a host PC, ADB, Termux, or Python.

Shows the current IMEI, lets you type a new one, and remembers the last 5 you've used so you can flip back. Requires root (Magisk).
...
2. AES-128-ECB decrypt the block at offset `0x40`, pull the BCD IMEI from the first 8 plaintext bytes — that's what shows in the "Current IMEI" field.
3. On change: re-encode the new IMEI, recompute the MD5-XOR checksum, re-encrypt, write the patched 384 bytes to the app's cache dir.
4. `su -c "mount -o remount,rw …"`, `cp` into place, `chmod 660`, `chown root:system`.
5. Offer to reboot — the modem caches the old IMEI until then.

The Java port lives in [`ImeiCrypto.java`](src/com/flipphoneguy/imeiswitcher/ImeiCrypto.java) and is a direct translation of [`imei_tool.py`](https://github.com/alltechdev/f21-imei-switcheroo/blob/main/imei_tool.py) (no partition-image mode — the app only ever touches the live `LD0B_001`). The on-device steps are in [`RootRunner.java`](src/com/flipphoneguy/imeiswitcher/RootRunner.java) and mirror the `su -c` calls in `termux_patch.sh` / `live_patch.sh`.

## History

The app remembers the last 5 IMEIs you've applied (most-recent first, deduplicated) in `SharedPreferences`. Tap **Use** to refill the input field and switch back.
